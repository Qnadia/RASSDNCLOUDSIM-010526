 ...
 * one logical channels (class Channel).
...
public class Channel {
	...
	public boolean adjustDedicatedBandwidthAlongLink() {
		...
		if (factor < 1.0) {
			System.err.println("Channel.adjustDedicatedBandwidthAlongLink(): " + this
					+ ": Cannot allocate requested Bw(" + this.getRequestedBandwidth() + "). Allocate only "
					+ requestedBandwidth);
		}
	 ...
	 * long processedThisRound =
	 * Math.round(timeSpent*getAllocatedBandwidthPerTransmission());
