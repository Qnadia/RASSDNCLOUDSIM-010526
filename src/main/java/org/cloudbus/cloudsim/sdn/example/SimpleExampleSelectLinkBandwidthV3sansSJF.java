...
public class SimpleExampleSelectLinkBandwidthV3sansSJF extends SimpleExample {
	...
	public static void main(String[] args) {
		...
		try { 
			...
			if(!SimpleExampleSelectLinkBandwidthV3sansSJF.logEnabled) 
				Log.disable();
	...
	// 					workloadList.sort((w1, w2) -> {
	// 						// Vérifier si les requêtes sont nulles
	// 						if (w1.request == null || w2.request == null) {
	// 							logWriter.println("Skipping workload comparison due to null request.");
	// 							return 0; // Ne pas changer l'ordre si une requête est nulle
	// 						}
	
	// 						// Obtenir le temps attendu de la première activité
	// 						double time1 = w1.request.getActivities().isEmpty() ? Long.MAX_VALUE : w1.request.getActivities().get(0).getExpectedTime();
	// 						double time2 = w2.request.getActivities().isEmpty() ? Long.MAX_VALUE : w2.request.getActivities().get(0).getExpectedTime();
