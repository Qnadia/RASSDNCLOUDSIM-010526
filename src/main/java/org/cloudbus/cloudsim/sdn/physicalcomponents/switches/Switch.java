...
public class Switch implements Node {
	...
	public Switch(String name, long bw, long iops, int upports, int downports) {
		...
		this.powerMonitor.setEntityId(this.address);
	}
	...
	public Node getVMRoute(int src, int dest, int flowId) {
		...
		if (route == null) {
			this.printVMRoute();
			System.err.println(
					"############ Switch.getVMRoute() No route (might be due to error, or dynamic route updating..):" +
	...
	private PowerUtilizationMonitor powerMonitor = new PowerUtilizationMonitor(
			new PowerUtilizationEnergyModelSwitchActivePort(), this.getId());
	...
	/* MAJ Nadia */
	// Ajout Nadia pour compatibilité avec CloudSim logging
	public int getId() {
