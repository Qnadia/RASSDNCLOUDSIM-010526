...
// 	}
	...
	public double getCapacity(List<Double> mipsShare) {
		if (mipsShare == null || mipsShare.isEmpty()) {
			System.err.println("⚠️ [ERROR] mipsShare est null ou vide. Utilisation d'une valeur par défaut.");
			return vmMips; // Retourne la capacité totale de la VM comme valeur par défaut
		}
	...
	protected void processTimeout(double currentTime) {
		if (timeoutLimit <= 0 || !Double.isFinite(timeoutLimit)) return;
	
		List<ResCloudlet> timeoutCloudlets = new ArrayList<>();
		double timeoutThreshold = currentTime - timeoutLimit;
	
		for (ResCloudlet rcl : getCloudletExecList()) {
			// Ne considérer que les cloudlets non terminés
			if (rcl.getRemainingCloudletLength() > 0 && rcl.getCloudletArrivalTime() < timeoutThreshold) {
				timeoutCloudlets.add(rcl);
			}
		}
	
		getCloudletFailedList().addAll(timeoutCloudlets);
		getCloudletExecList().removeAll(timeoutCloudlets);
	}
	// protected void processTimeout(double currentTime) {
