...
// import org.cloudbus.cloudsim.sdn.sfc.ServiceFunctionForwarder; // removed: sfc package deleted
...
public class ChannelManager {
	...
	// protected ServiceFunctionForwarder sfcForwarder = null; // removed: sfc
	// package deleted
	...
	/*
	 * public ChannelManager(NetworkOperatingSystem nos, VirtualNetworkMapper
	 * vnMapper,
	 ...
	 * // Validation du flowId
	 * if(flowId < 1){
	 * System.err.println("createChannel(): flowId invalide: " + flowId +
	 * " pour le flux " + src + "->" + dst);
	 * throw new IllegalArgumentException("createChannel(): flowId invalide: " +
	 * flowId);
	 * }
	 * 
	 * // Récupérer la bande passante requise depuis la map
	 * long reqBw = nos.getRequestedBandwidth(flowId);
	 * if(reqBw == 0) {
	 * throw new RuntimeException("reqBW cannot be zero for dedicated channels!!" +
	 * flowId);
	 * }
	 * 
	 * // Construire les nœuds et les liens du chemin
	 * List<Node> nodes = new ArrayList<>();
	 * List<Link> links = new ArrayList<>();
	 * nos.buildNodesLinks(src, dst, flowId, srcNode, nodes, links);
	 * 
	 * // Créer le canal
	 * Channel channel = new Channel(flowId, src, dst, nodes, links, reqBw,
	 * (SDNVm)NetworkOperatingSystem.findVmGlobal(src),
	 * (SDNVm)NetworkOperatingSystem.findVmGlobal(dst));
	 * System.out.println("Channel créé: " + channel);
	 * 
	 * return channel;
	 * }
	 */
	...
	public Channel createChannel(int src, int dst, int flowId, Node srcNode) {
		...
		/* MAJ Nadia */
		// Vérifier si la politique autorise le recalcul dynamique
		if (nos.getLinkSelectionPolicy().isDynamicRoutingEnabled()) {
	...
	public boolean updatePacketProcessing() {
		System.out.println("updatePacketProcessing");
	...
	public void updateMonitor(double monitoringTimeUnit) {
		...
		for (Channel ch : channelTable.values()) {
			ch.updateMonitor(CloudSim.clock(), monitoringTimeUnit);
			// sfcForwarder.updateSFCMonitor(ch.getSrcId(), ch.getDstId(), ch.getChId(),
			// processedBytes); // removed: sfc package deleted
		}
