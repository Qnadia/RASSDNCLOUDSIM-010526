...
public class VmAllocationPolicyEx extends VmAllocationPolicy implements PowerUtilizationMaxHostInterface {
	...
	// Removed redundant global hostTotal variables to support heterogeneous hosts
...
//			getFreePes().set(idx, getFreePes().get(idx) - requiredPes);
	...
	public double[] buildFreeResourceMetric(List<? extends Host> hosts) {
		...
		for (int i = 0; i < hosts.size(); i++) {
			...
			double mipsFreePercent = (double)getAvailableMips(h)/ h.getTotalMips(); 
			double bwFreePercent = (double)getAvailableBw(h) / h.getBw();
			double ramFreePercent = (double)h.getRamProvisioner().getAvailableRam() / h.getRam();
